...
pub async fn get_queue_timeout_secs(client: &Client, queue_url: &str) -> u16 {
        ...
        .await
    {
        ...
        Ok(gqa) => match gqa.attributes() {
            None => {
                tracing::debug!(
                    "Queue {queue_url}: no attrs, using default {QUEUE_TIMEOUT_SECS} secs"
                );
                QUEUE_TIMEOUT_SECS
            }
    ...
    if let Some(rcpt_handle) = m.receipt_handle().map(|s| s.to_owned()) {
        ...
        let join_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = cmv {
                    tracing::error!(
                        "Message {msg_id}: failed to update lease: {}",
                        e.to_string()
                    );
                }
            }
